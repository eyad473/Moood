...
const APP_RELEASE_VERSION = "52.0";
...
async function renderDisplayAccounts(){const box=document.getElementById("displayAccountsList");if(!box)return;box.innerHTML='<div class="empty">جاري تحميل الحسابات...</div>';try{const r=await authFetch("/auth/display-accounts",{method:"GET"});box.innerHTML=r.accounts.length?r.accounts.map(a=>`<div class="display-account-row ${a.enabled?"":"disabled"}"><div class="display-account-main"><b>${esc(a.fullName)}</b><small>اسم المستخدم: <strong dir="ltr">${esc(a.username)}</strong> · ${a.enabled?"مفعّل":"موقوف"}</small></div><div class="display-account-actions"><button class="btn" onclick="toggleDisplayAccount(${a.id},${a.enabled})">${a.enabled?"إيقاف":"تفعيل"}</button><button class="btn" onclick="resetDisplayPassword(${a.id},'${esc(a.username)}')">تغيير كلمة المرور</button></div></div>`).join(""):'<div class="empty">لا توجد أجهزة عرض مضافة.</div>'}catch(e){box.innerHTML=`<div class="notice">${esc(e.message||"تعذر تحميل الحسابات")}</div>`}}
